	...
	<xsl:template match="cerif:Project">
		<dim:dim>
			...
			<dim:field mdschema="oairecerif" element="funder">
                <xsl:if test="cerif:Funded/cerif:By/cerif:OrgUnit/@id">
                    <xsl:attribute name="authority"><xsl:value-of select="concat($idPrefix,cerif:Funded/cerif:By/cerif:OrgUnit/@id)"/></xsl:attribute>
                </xsl:if>
                <xsl:value-of select="cerif:Funded/cerif:By/cerif:DisplayName"/>
            </dim:field>
