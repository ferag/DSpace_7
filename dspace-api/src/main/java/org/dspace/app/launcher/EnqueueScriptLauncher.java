 ...
 * @author Mark Diggory
 */
public class EnqueueScriptLauncher {

    private static final Logger log = Logger.getLogger(EnqueueScriptLauncher.class);
    ...
    public static int handleScript(String[] args, Document commandConfigs,
        ...
        if (script != null) {
            ...
            status = enqueueScript(kernelImpl, args, dSpaceRunnableHandler, script, currentUser);
        } else {
    ...
    protected static int runOneCommand(DSpaceKernelImpl kernelImpl, Document commandConfigs, String[] args) {
...
//                main.invoke(null, arguments);
                enqueueCommand(kernelImpl, commandConfigs, args);
    ...
    private static void enqueueCommand(DSpaceKernelImpl kernelImpl, Document commandConfigs, String[] args) {
        kernelImpl.getConfigurationService().getProperty("dspace.dir");
        SqsService.enqueueCommand(kernelImpl.getConfigurationService().getProperty("dspace.serviceid"), args,
            UUID.randomUUID().toString());
    }

    private static int enqueueScript(DSpaceKernelImpl kernelImpl, String[] args,
        DSpaceRunnableHandler dSpaceRunnableHandler, DSpaceRunnable script, EPerson currentUser) {
        SqsService.enqueueCommand(kernelImpl.getConfigurationService().getProperty("dspace.serviceid"),
            args, UUID.randomUUID().toString());
        return 0;
    }

    private static boolean copyBin(DSpaceKernelImpl kernelImpl) {
//        try {
//            return Runtime.getRuntime().exec(String.format(
//                kernelImpl.getConfigurationService().getProperty("dspace.package_script_dir")
//                + " -n" + kernelImpl.getConfigurationService().getProperty("dspace.serviceid")
//                + " -s" + kernelImpl.getConfigurationService().getProperty("dspace.dir"))).waitFor() == 0;
        return true;
//        } catch (IOException | InterruptedException e) {
//            System.err.println("error copying binary folders to DSpace: "
//                + String.format("/usr/local/dspacePackager -n"
//                + kernelImpl.getConfigurationService().getProperty("dspace.serviceid")
//                + " -s" + kernelImpl.getConfigurationService().getProperty("dspace.dir")));
//            return false;
//        }
    }
